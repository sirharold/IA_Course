...
def load_env_data() -> dict[str, list[dict]]:
    ...
    for env_name in SUPPORTED_ENVS:
        file_path = DATA_DIR / f"{env_name}.json"
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/health":
            json_response(
                ...
                {
                    "status": "ok",
                    "service": "advanced-env-api",
                    "supported_environments": sorted(SUPPORTED_ENVS),
                },
            )
            return

        if parsed.path != "/v1/customer-offer":
        ...
        if caller_env not in SUPPORTED_ENVS:
            json_response(
                ...
                {
                    "error": "bad_request",
                    "message": "X-Caller-Environment must be development, staging or production",
                },
            )
            return
        ...
        if not record:
            json_response(
                ...
                {
                    "error": "not_found",
                    "message": f"user {user_id} not found for {caller_env}",
